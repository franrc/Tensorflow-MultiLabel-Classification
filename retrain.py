...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  """
  tf.logging.info("Looking for images")
  ...
  for dirpaths, dirnames, filenames in os.walk(image_dir):

    if not dirnames:
    	splittedPath=dirpaths.split(os.path.sep)

    	numLabels = len(splittedPath) - 1
    	imageLabels = [None] * numLabels

    	for x in range(0, numLabels):
			imageLabels[x] = splittedPath[x+1]
    	...
    	for extension in extensions:
    		file_glob = os.path.join(dirpaths, '*.' + extension)
    	...
    	for imagePath in file_list:
    		...
    		# the same leaf.
    		hash_name = re.sub(r'_nohash_.*$','',imageName)
    		...
    		if percentage_hash < validation_percentage:
    			validation_images.append(imageName)
    		elif percentage_hash < (testing_percentage + validation_percentage):
    		  testing_images.append(imageName)
    		else:
    		  training_images.append(imageName)

    	if not dirpaths.endswith("/"):
    		dirpaths = dirpaths + "/"

    	result[dirpaths] = {
			'truth_labels': imageLabels,
    	...
    	print(dirpaths)
    	print("\n")
    	print(result[dirpaths])
    	print("\n\n\n\n\n\n")
  return result


def get_image_path(image_lists, dirPath, index, image_dir, category):
  """Returns a path to an image for a label at the given index.
  Args:
	image_lists: OrderedDict of training images for each label.
	dirPath: Subdirectory where images are located.
  ...
  """
  if dirPath not in image_lists:
    tf.logging.fatal('Directory path does not exist %s.', dirPath)
  
  path_lists = image_lists[dirPath]

  if category not in path_lists:
    tf.logging.fatal('Category does not exist %s.', category)
  
  category_list = path_lists[category]
  
  if not category_list:
    tf.logging.fatal('DirPath %s has no images in the category %s.',
                     dirPath, category)
...
def get_bottleneck_path(image_lists, dirPath, index, bottleneck_dir,
						category, module_name):
  """Returns a path to a bottleneck file for a label at the given index.
  Args:
	image_lists: OrderedDict of training images for each label.
	dirPath: Absolute directory path to list of images.
  ...
  Returns:
	File system path string to an image that meets the requested parameters.
  """
  module_name = (str(module_name).replace('://', '~')  # URL scheme.
				 .replace('/', '~')  # URL and Unix paths.
				 .replace(':', '~').replace('\\', '~'))  # Windows paths.
  return get_image_path(image_lists, dirPath, index, bottleneck_dir,
						category) + '_' + str(module_name) + '.txt'
...
def create_bottleneck_file(bottleneck_path, image_lists, dirPath, index,
						   image_dir, category, sess, jpeg_data_tensor,
						   decoded_image_tensor, resized_input_tensor,
						   bottleneck_tensor):
  """Create a single bottleneck file."""
  tf.logging.debug('Creating bottleneck at ' + bottleneck_path)
  image_path = get_image_path(image_lists, dirPath, index,
							  dirPath, category)
...
def get_or_create_bottleneck(sess, image_lists, dirPath, index, image_dir,
  ...
  Args:
	...
	image_lists: OrderedDict of training images for each label.
	dirPath: Absolute path of image list.
  ...
  """
  label_lists = image_lists[dirPath]
  sub_dir_path = os.path.join(bottleneck_dir, dirPath)

  ensure_dir_exists(sub_dir_path)	
  bottleneck_path = get_bottleneck_path(image_lists, dirPath, index,
										bottleneck_dir, category, module_name)
  if not os.path.exists(bottleneck_path):
	create_bottleneck_file(bottleneck_path, image_lists, dirPath, index,
						   image_dir, category, sess, jpeg_data_tensor,
						   decoded_image_tensor, resized_input_tensor,
						   bottleneck_tensor)
...
def cache_bottlenecks(sess, image_lists, imagePath, bottleneck_dir,
  ...
  for dirPath, label_lists in image_lists.items():
	for category in ['training', 'testing', 'validation']:
	  category_list = label_lists[category]
	  for index, unused_base_name in enumerate(category_list):
		get_or_create_bottleneck(
			sess, image_lists, dirPath, index, image_dir, category,
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
								  ...
								  decoded_image_tensor, resized_input_tensor,
								  bottleneck_tensor, module_name, labels):
  ...
  if how_many >= 0:
	...
	for unused_i in range(how_many):
	  ...
	  dirPath = list(image_lists.keys())[label_index]

	  image_index = random.randrange(MAX_NUM_IMAGES_PER_CLASS + 1)
	  image_name = get_image_path(image_lists, dirPath, image_index,
								  image_dir, category)

	  bottleneck = get_or_create_bottleneck(
		  sess, image_lists, dirPath, image_index, image_dir, category,
		  bottleneck_dir, jpeg_data_tensor, decoded_image_tensor,
		  resized_input_tensor, bottleneck_tensor, module_name)
	  ...
	  ground_truth = get_ground_truth(dirPath, image_lists[dirPath]['truth_labels'], labels)
	...
	for label_index, dirPath in enumerate(image_lists.keys()):
	  for image_index, image_name in enumerate(
		  image_lists[dirPath][category]):
		image_name = get_image_path(image_lists, dirPath, image_index,
									image_dir, category)
		bottleneck = get_or_create_bottleneck(
			sess, image_lists, dirPath, image_index, image_dir, category,
...
def get_ground_truth(dirPath, trueLabels, labels):
	if dirPath in CACHED_GROUND_TRUTH_VECTORS.keys():
		ground_truth = CACHED_GROUND_TRUTH_VECTORS[dirPath]
	else:
		...
		for label in labels:
			...
			idx += 1
		CACHED_GROUND_TRUTH_VECTORS[dirPath] = ground_truth
...
def get_random_distorted_bottlenecks(
  ...
  for unused_i in range(how_many):
	...
	image_path = get_image_path(image_lists, dirPath, image_index, dirPath,
								category)
	if not tf.gfile.Exists(image_path):
	  tf.logging.fatal('File does not exist %s', image_path)
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  return (train_step, cross_entropy, bottleneck_input, ground_truth_input,
		  final_tensor)
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  test_bottlenecks, test_ground_truth, test_filenames = (
	  get_random_cached_bottlenecks(train_session, image_lists,
									...
									bottleneck_tensor, FLAGS.tfhub_module, labels))
...
def generate_output_labels(image_lists):
	
	global labels

	for dirPath in image_lists.keys():
		labels.extend(image_lists[dirPath]['truth_labels'])    
...
def main(_):
  ...
  generate_output_labels(image_lists)
	  ...
	  if do_distort_images:
		(train_bottlenecks,
		 train_ground_truth) = get_random_distorted_bottlenecks(
			 sess, image_lists, FLAGS.train_batch_size, 'training', distorted_jpeg_data_tensor,
...
if __name__ == '__main__':
  ...
  parser.add_argument(
  	...
  	type=str,
  	default="labels.txt",
  	help='Where to find all possible labels of the model')
